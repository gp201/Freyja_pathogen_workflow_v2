...
def parser():
    ...
    parser = argparse.ArgumentParser(
        description='Extract lineage and strain columns from metadata file')
    parser.add_argument("-m", "--metadata_file",
                        help="Metadata file", required=True)
    parser.add_argument("-s", "--strain_col",
                        help="Strain column name", default='strain')
    parser.add_argument("-l", "--lineage_col",
                        help="Lineage column name", default='lineage')
...
def main():
    ...
    # switch the columns
    df = switch_columns(df[[args.strain_col, args.lineage_col]].copy(
    ), args.strain_col, args.lineage_col)
